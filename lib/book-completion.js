...
export async function refreshBookCompletionStatus(bookId) {
  // c.leaf_no may carry a "YYYY-" year prefix (e.g. "2026-5"); strip it
    ...
    `SELECT
       ...
       COUNT(*) FILTER (
         WHERE c.accounted = true
           AND regexp_replace(trim(c.leaf_no::text), '^[0-9]{4}-', '') ~ '^-?[0-9]+$'
           AND regexp_replace(trim(c.leaf_no::text), '^[0-9]{4}-', '')::bigint
             BETWEEN b.leaf_no_from AND b.leaf_no_to
